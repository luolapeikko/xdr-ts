...
import {RpcAuthFlavor} from '../src/rpc/RpcAuth';
import {RpcMsgType, rpcCallSchemaModel} from '../src/rpc/RpcCall';
...
describe('RpcBind GETTIME', () => {
	...
	it('should send GETTIME call to rpcbind on localhost:111 (UDP) and receive response', async () => {
		...
		console.log('UDP Payload Hex:', payload.toString('hex'));
		...
		console.log('UDP Response Hex:', response.toString('hex'));
		...
		console.log('Remote Time (UDP):', new Date(timestamp * 1000).toLocaleString());
	...
	it('should send GETTIME call to rpcbind on multiple addresses (TCP) and receive response', async () => {
		...
		if (!response) {
			throw new Error('Failed to connect to any host via TCP');
		}

		console.log('TCP Response Hex:', response.toString('hex'));
		expect(response.length).toBeGreaterThan(0);
		...
		const timestamp = resXdr.readUInt();
		console.log('Remote Time (TCP):', new Date(timestamp * 1000).toLocaleString());
		expect(timestamp).toBeGreaterThan(0);
	});
});
