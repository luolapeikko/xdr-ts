...
describe('XdrBuffer', () => {
	describe('Constructor', () => {
		...
		it('should create a buffer with zero size when passed 0', () => {
			const xdr = new XdrUint8Buffer(0);
			expect(xdr.size()).toBe(0);
		});
	});
	...
	describe('Getters', () => {
		it('should return current pointer position', () => {
			const xdr = new XdrUint8Buffer(10);
			expect(xdr.currentPointer).toBe(0);
			xdr.writeByte(1);
	...
	describe('ToString', () => {
		it('should return correct string representation', () => {
			const xdr = new XdrUint8Buffer(100);
			expect(xdr.toString()).toBe('XdrUint8Buffer{pointer=0, size=100}');
		});

		it('should update toString after writes', () => {
			const xdr = new XdrUint8Buffer(100);
			xdr.writeInt(42);
			expect(xdr.toString()).toBe('XdrUint8Buffer{pointer=4, size=100}');
		});
	...
	describe('Error handling', () => {
		...
		it('should throw when reading short beyond bounds', () => {
			const xdr = new XdrUint8Buffer(1);
			expect(() => {
				xdr.readShort();
			}).toThrow('xdr buffer too short');
		});
