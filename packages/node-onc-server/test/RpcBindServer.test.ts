import {RpcBindClient} from '@luolapeikko/node-onc-client';
import {type RpcProcedure, RpcProcUnavailError, RpcRequest, RpcTcpTransport, RpcUdpTransport} from '@luolapeikko/xdr-ts';
...
describe('RpcBindServer', () => {
	...
	describe('RpcTcpTransport', () => {
		...
		it('should get address via TCP', async () => {
			...
			expect(addr).toBe(`127.0.0.1.${testPort >> 8}.${testPort & 0xff}`);
		});
	});
	...
	describe('RpcUdpTransport', () => {
		...
		it('should get address via UDP', async () => {
			...
			expect(addr).toBe(`127.0.0.1.${udpPort >> 8}.${udpPort & 0xff}`);
		});
	});
