...
const defaultOptions = {
	...
	rpcbindOwner: 'owner',
	autoRegister: true,
} as const satisfies Required<TinyServiceOptions>;

export class TinyService {
	private readonly options: Required<TinyServiceOptions>;
	private udpSocket: dgram.Socket | undefined;
	private tcpServer: net.Server | undefined;
	private rpcbindTransport: RpcUdpTransport | undefined;
	...
	public get udpUaddr() {
		return this.toUniversalAddress(this.udpPort);
	}

	public get tcpUaddr() {
		return this.toUniversalAddress(this.tcpPort);
	}
	...
	private toUniversalAddress(port: number): string {
		const upper = (port >> 8) & 0xff;
		const lower = port & 0xff;
		return `${this.options.host}.${upper}.${lower}`;
	}
	...
	private getRpcbindClient(): RpcBindClient {
		if (!this.rpcbindClient) {
			this.rpcbindTransport = new RpcUdpTransport({host: this.options.rpcbindHost, port: this.options.rpcbindPort});
